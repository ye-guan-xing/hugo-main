...
```bash
# 把所有改动的文件加入“待提交清单”（.代表所有文件）
git add .
# 保存改动并加备注（备注要清晰，新手别乱写）
git commit -m "合并master分支代码，同步最新内容"
# 把本地z1的代码推到云端z1分支（关键！不然云端代码还是旧的）
git push origin z1
```

✅ 新手解读：这三步相当于“保存草稿 → 给草稿写备注 → 把草稿上传到云端”，确保你的修改不会丢。

## 关键原则：为什么绝对不能在 master 分支提交代码？
