...
| 页面 | 写法 | 事件 | 说明 |
|---|---|---|---|
| 阅读信件报告页 | C + G | `biz_hs_page`、`biz_hs_bottom_show`、`page` | 心跳 3s、`visibilitychange`、滚动到底上报 |
| 阅读诊断报告页 | C + G | `biz_hs_page/show`、`page/show` | 11 个模块曝光时长，详见报告页埋点规格文档 |
| 阶段测评报告页 | C + G | `biz_hs_page/show/click` | 4 模块曝光，见报告页埋点规格文档 |
| 简化版报告页 | C | `biz_hs_*` | 简化版报告 |
...
| 页面 | 写法 | 调用点数 | 事件 |
|---|---|---|---|
| 每日资讯订阅页 | A 直接调 | 23 | page / show / click 全覆盖 |
| 内容卡页面 | B this.track 封装 | ~9 | page / show / click |
| 内容卡详情页 | B this.track 封装 | ~3 | page / click |
...
| 课程计划页 | A | 课程计划 |
| 音频广场组件 | ② $tracker | `biz_hs_knowledge_publishing_course_play` 音频播放时长 |
...
| 运营活动页 | ② $tracker | `biz_hs_h5_campaign_view`、`biz_hs_sy_page_view` |
| 活动宣传组件 | ② $tracker | `biz_hs_h5_info_click` |
| 底部活动按钮组件 | ② $tracker | `biz_hs_h5_campaign_click` |
| 弹窗组件 | ② $tracker | 弹窗点击 |
| 订阅广场组件 | ② $tracker | 订阅广场 |
| 订阅包组件 | ② $tracker | 订阅包 |
...
| 页面 | 写法 | 点位 |
|---|---|---|
| 签到活动页 | A + ② $tracker | `biz_hs_click` 签到点击 |
| 签到视频列表组件 | ② $tracker | `biz_hs_click`、`biz_hs_show` |
| 签到成功组件 | ② $tracker | `biz_hs_show` 签到成功 |
| 裂变页 | A | 裂变页 |
| 兑换课程页 | A | 12 处 click |
| 领礼物页 | A | 领礼物 |
| 券成功页 | A | 券成功 |
...
| 训练营新群提示页 | A | 2 处 |
| 训练营群提示页 | A | 1 处 |
| 训练营简化结果页 | A + classKey | `camp_cfg` 远程配置 |
| 训练营简化阅读报告页 | A + classKey | `camp_cfg` |
| 训练营短信二维码页 | A | 3 处 |
| 实体勋章页系列（首页/详情/成功/好友/营地） | A + ② $tracker | 勋章页系列 |

> `classKey: 'camp_cfg'` 是远程配置开关，控制是否开全埋点 / 热力图圈选（详见 [三大埋点系统 ②](/p/tracking-systems/)）。其它 classKey：`h5_exhibit_cfg`、`AUDIO_CARD`、`mall`、`h5_review`。
...
| 会员卡逻辑 | ② $tracker | 会员卡 `biz_hs_${event}` |
...
| 文档 | 内容 |
|---|---|
| 项目埋点规格文档 | 每日资讯 + 内容卡 列表 / 详情页全量埋点字段表 |
